...
namespace ServerEngine.MessageBroker
{
    internal static class Broker
    {
        ...
        {
            var sslCertThumbprint = "40B7C202B3B3D47CAD5ACA11BBF5C86D87C8A89F";
            // SSL self signed certificate creation notes.
            // 1. Start an admin powershell
            // 2. New-SelfSignedCertificate -DnsName [Your-Computer/Server-Name] -CertStoreLocation "cert:\LocalMachine\My" -KeySpec KeyExchange
            ...
            // 3. Copy-pasta the thumbprint here.
            // 4. Open MMC/Manage computer certs, locate the cert in personal and give the cert a friendly name if desired
            // 5. Copy the cert from Personal to Trusted Root Certification Authorities (Also in localmachine)
            // 6. Right click on the certificate in LocalMachine\Personal and pick All Tasks->Manage Private Keys
            //    Add your current login explicitly here with full control.

            // Get the ssl cert from the local cert store.
            X509Store store = new X509Store(StoreLocation.LocalMachine);
            store.Open(OpenFlags.MaxAllowed);
            var foundCerts = store.Certificates.Find(X509FindType.FindByThumbprint, sslCertThumbprint, false);
